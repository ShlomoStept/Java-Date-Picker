...
Author: Shlomo Stept
Project: This project aimed at providing a Java Swing Date-picker Calendar JPanel
        -- to place within another applications JFrame use the other github class
        --- the other file Java Swing Date-picker Calendar JFrame (opens up a self sufficient window application), to be used on its own

***** 1.0 ****** IMPORTANT !!!!!!!!!!!!!!!!!!!
TO USE IN ANOTHER APPLICATION USE:  (use the ---> getFrame()  ---> method to get the frame )
1. calender = new JavaSwingDatePickerJPanelGUI();
2. JPanel calenderPanel = new JPanel();
3. calenderPanel = calender.getFrame();
4. calenderPanel.setBounds(20, 88, 330, 300);  --> recommended window size based on current sizes of components -- change this if you change any component sizes
5. "SOMEVARIABLENAME".add(calenderPanel);  -->  sometimes if you do this within another class just -->  add(calenderPanel);



***** 2.0 ****** IMPORTANT !!!!!!!!!!!!!!!!!!!
  When a date is selected on the calendar the data-field/variable "LocalDate inputDate" updates to the selected date
    for your use this in your application



Bonuses:
 -- Editing the (A) colors, (B) font style and sizes, and most importantly (C) the Size of each component
        can easily be done via the data fields (global variables) in the beginning of this file
...
public class JavaSwingDatePickerJPanelGUI implements Runnable {
...
//---------------------------------------------------------------------------------------------------------------------
    ...
    // 2.0 --> ::: CONSTRUCTOR :::
    //-----------------------------------------------------------------------
    public JavaSwingDatePickerJPanelGUI() {
